...
void	ft_render_cadete_soldadier(t_data *data)
{
	int	i;

	data->cadete.img = mlx_xpm_file_to_image(data->mlx,
			data->cadete.img_old, &i, &i);
	mlx_put_image_to_window(data->mlx, data->win, data->cadete.img,
		data->cadete.pos_x, data->cadete.pos_y);
	data->soldier.img = mlx_xpm_file_to_image(data->mlx,
			"textures/soldado.xpm", &i, &i);
	mlx_put_image_to_window(data->mlx, data->win, data->soldier.img,
		data->soldier.pos_x, data->soldier.pos_y);
	mlx_destroy_image(data->mlx, data->soldier.img);
...
void	render_map(t_data *data, int time)
{
	...
	ft_render_cadete_soldadier(data);
}
